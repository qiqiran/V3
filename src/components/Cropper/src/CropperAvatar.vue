  ...
  <div :class="getClass" :style="getStyle">
    <div :class="`${prefixCls}-image-wrapper`" :style="getImageWrapperStyle" @click="openModal">
      <div :class="`${prefixCls}-image-mask`" :style="getImageWrapperStyle">
        <CloudUploadOutlined :size="getIconWidth" :style="{ ...getImageWrapperStyle, fontSize: getIconWidth, color: '#d6d6d6' }" />
      </div>
      <img :src="sourceValue" v-if="sourceValue" alt="avatar" />
    </div>
    <Button v-bind="btnProps" :class="`${prefixCls}-upload-btn`" @click="openModal" v-if="showBtn">{{ btnText ? btnText : `选择图片` }}</Button>
